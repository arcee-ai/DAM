...
class DAMTrainer(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        # Compute KL divergence loss for each item in the batch
        temperature = 2.0
        loss_1 = F.kl_div(
            F.log_softmax(merged_logits_1 / temperature, dim=-1),
            F.softmax(logits_1 / temperature, dim=-1),
            reduction='batchmean'
        ) * (temperature ** 2)

        loss_2 = F.kl_div(
            F.log_softmax(merged_logits_2 / temperature, dim=-1),
            F.softmax(logits_2 / temperature, dim=-1),
            reduction='batchmean'
        ) * (temperature ** 2)

        loss_3 = F.kl_div(
            F.log_softmax(merged_logits_3 / temperature, dim=-1),
            F.softmax(logits_3 / temperature, dim=-1),
            reduction='batchmean'
        ) * (temperature ** 2)

        # Compute similarity loss and regularization across the batch
        similarity_loss = torch.tensor(0.0, device=device)
        similarity_reg_loss = torch.tensor(0.0, device=device)
        for module in model.modules():
            if isinstance(module, (DAMLinearLayer, DAMEmbeddingLayer)):
                similarity_reg_loss += module.compute_mergers_similarity(lambda_coef_reg=None).to(device)

        # Total loss (sum across the batch)
        total_loss = (loss_1 + loss_2 + loss_3) + similarity_reg_loss
