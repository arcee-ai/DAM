...
def compute_dam_loss_for_lm(model, prompt_a, prompt_b, prompt_c, lambda_coef=0.01, temperature=2.0):
    ...
    # The cosine similarity loss, when applied to the merging coefficients, is indeed encouraging the coefficients to 
    # be orthogonal (or as close to orthogonal as possible) for each module.
    # Example integration within the loss computation process

    similarity_reg_loss = torch.tensor(0.0)
    for module in base_model.modules():
        if isinstance(module, DAMLayer):
            similarity_reg_loss += module.compute_mergers_similarity(lambda_coef_reg=0.01).to(similarity_reg_loss.device)
