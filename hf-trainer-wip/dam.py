...
class DAMLayer(nn.Module):
    def __init__(
        ...
        # The residuals (differences) between the weights of linear_a,linear_b, linear_c and the base_linear weights are 
        # computed and stored as buffers. These residuals will be used to adjust the base_linear weights during the forward pass
        ...
        #  If all the linear layers have biases, their residuals (differences from the base bias) are computed and stored as buffers. 
        ...
        # control the weighting of the residuals  (differences from the base) from the different models. They are initialized with the provided init_merger_values.
    ...
    def set_forward_type(self, type: str = "merge"):
        ...
        type (str): Specifies the type of forward pass. The options are:
            ...
            - "weight_1": Use the base model's weights combined with the residual from `linear_a`.
            - "weight_2": Use the base model's weights combined with the residual from `linear_b`.
            - "weight_3": Use the base model's weights combined with the residual from `linear_c`.
    ...
    def compute_mergers_similarity(self):
        """
        This method computes the cosine similarity between the merger parameters (merger_1, merger_2, merger_3). It returns the average 
        similarity between these parameters, which can be used to measure how similarly the different models are contributing to the final merged weights.
        """
    ...
    def get_dam_weight(self):
        """
        This method computes the merged weights by adding the base weights to the weighted residuals 
